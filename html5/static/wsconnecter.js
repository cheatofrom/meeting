...
function WebSocketConnectMethod( config ) { //定义socket连接方法类
	...
	function onError( e ) {
		...
		info_div.innerHTML="连接错误: " + e.message;
		console.error("WebSocket连接错误:", e);
		
		// 检查是否是SSL证书问题
		if (document.getElementById('wssip').value.startsWith("wss://")) {
			info_div.innerHTML = "WebSocket连接失败，可能是SSL证书问题。请点击上方链接手动授权SSL证书，或尝试使用ws://替代wss://";
			console.log("可能是SSL证书问题，建议用户手动授权或使用ws://");
		}
