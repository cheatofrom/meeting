...
function toggleWsProtocol() {
	var wssipInput = document.getElementById('wssip');
	var currentUrl = wssipInput.value;
	
	if (currentUrl.startsWith("wss://")) {
		// 切换到ws://
		wssipInput.value = currentUrl.replace("wss://", "ws://");
		console.log("已切换到WS协议");
		info_div.innerHTML = "已切换到WS协议（不安全但可能解决证书问题）";
	} else if (currentUrl.startsWith("ws://")) {
		// 切换回wss://
		wssipInput.value = currentUrl.replace("ws://", "wss://");
		console.log("已切换到WSS协议");
		info_div.innerHTML = "已切换到WSS协议（安全连接）";
	}
...
function getJsonMessage( jsonMsg ) {
	...
	var speaker_id=JSON.parse(jsonMsg.data)['speaker_id'];
	
	// 如果有说话人ID，添加到识别结果前面
	if(speaker_id !== undefined) {
		rectxt = "[说话人" + speaker_id + "] " + rectxt;
	}
